  ...
  accountPanel.innerHTML = `
    ...
    <div class="account-panel__text">
      ...
      <div class="account-panel__sub">${escapeHtml(
        account.guest
          ? "Your save lives in this browser. Clearing site data would lose it."
          : account.detail
      )}</div>
    </div>
    ...
    ${
      account.guest
        ...
        : `<button class="btn btn--danger" data-action="signout" type="button">
             ${icons.logout}
             Sign out
           </button>`
    }
      ...
      const choice = await confirmDialog({
        ...
        body: `
          ...
          <p style="margin-top:12px">
            You can still sign in with Google, but this browser's
            guest progress will not move across.
          </p>
        `,
  ...
  accountPanel
    ...
    ?.addEventListener("click", async () => {
      const choice = await confirmDialog({
        ...
        body: `
          <p>
            Your save stays on your Google account. Signing back in
...
onAccountChange((event, user) => renderAccount(user));
...
async function loadAccount() {
  ...
  // Makes sure a first-time visitor still ends up with a guest
  // account when they land straight on Settings.
  await ensurePlayerAuth();

  const { data: refreshed } = await supabase.auth.getSession();

  renderAccount(refreshed.session?.user ?? null);
}
